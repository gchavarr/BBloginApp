...
import com.bb.loginapp.config.KafkaProperties;
...
class BBloginAppKafkaResourceIT {
    ...
    void setup() {
        ...
        BBloginAppKafkaResource kafkaResource = new BBloginAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/b-blogin-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/b-blogin-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
